...
- Sessions and archived sessions persist across bridge reconnects. Retention defaults to 30 days, runs at startup and daily, and may delete only an inactive, confidently owned old thread after compatible goal cleanup (when available) and native `thread/delete`; live or uncertain bridges, leases, sidecars, malformed journals, and partial failures must defer deletion. Background job records remain connection-local
- Native goals use `thread/goal/set|get|clear`, not prompt injection. Only the empirically fingerprinted goal-store layout may be shared into isolated `CODEX_SQLITE_HOME`; recognized compatible Codex versions with an unverified goal layout keep ordinary turns available but fail goal-bearing calls closed, while an unknown or too-old CLI fails before child startup. Preserve SQLite's proven main-DB symlink behavior and validate/chmod the canonical DB, WAL, and SHM opportunistically
