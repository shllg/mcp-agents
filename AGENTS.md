...
- Native subagents are OFF by default and session-scoped. The isolated config writes `[features] multi_agent = false` plus — version-gated — `[agents] enabled = false`: on Codex >= 0.145.0 the stabilized feature flag alone does NOT remove the collab tools (verified empirically; only `agents.enabled = false` does), while on 0.102–0.144 a boolean under `[agents]` routes into the flattened custom-role map and HARD-FAILS config parsing, so the bridge probes `codex --version` once at startup (`readCodexBinaryVersion`/`codexSupportsAgentsEnabledKey`) and emits the `[agents]` line only for >= 0.145.0 (unknown version assumes modern; older versions still honor the feature flag by itself). A `codex`/`codex-start` call with `allow_subagents: true` re-enables them for that session only via per-call config overrides (`features.multi_agent` always, `agents.enabled` under the same version gate); replies inherit it like `sandbox` and reject the arg. The `[mcp_servers]` strip is untouched either way, so enabled subagents remain Codex-only in-process workers with no bridge re-entry or cross-LLM reach; they inherit the session's `sandbox_mode` + `approval_policy` (shared-workspace concurrency is the residual caveat). Multi-agent V2 reports lifecycle work as `sub_agent_activity` (narrated in progress alongside the older `collab_agent_*` names)
- `transformCodexToolCall` only receives already-validated calls. It preserves the curated initial-session native `model`, translates wrapper effort to native `config.model_reasoning_effort`, strips wrapper-only `allow_subagents` (only `true` injects the native overrides above), and strips/injects wrapper-only `goal`: initial goals become native `developer-instructions`, while reply goals become prompt reminders. Codex's native `/goal` is TUI-only. Accepted frames with nothing to transform (notably a goal-free `codex-reply`) MUST remain byte-for-byte unchanged. The stdin pump therefore buffers raw bytes and splits only on newline byte `0x0a`
- The **OUTBOUND** path replaces the native `codex`/`codex-reply` input schemas in a `tools/list` RESPONSE with the exact curated contracts while preserving other tool metadata. It also inspects foreground turn frames only while their response ids are outstanding so a correlated typed `unauthorized` event plus duplicate `isError` result can become one wrapper-owned `codex_auth_invalidated` result before either reaches the client. This remains a **contained latch**: steady state stays a raw byte-for-byte forwarder (`forwardChunk`), unrelated buffered frames retain their original bytes, and the latch closes when the matched response completes. `observeOutgoing` runs on ORIGINAL bytes and remains the sole authority for native in-flight/watchdog tracking. Invariants covered by stub tests: unrelated frames stay byte-for-byte; every complete buffered frame flushes even under backpressure; oversized frames forward raw; a mode-boundary straddle raw-skips the orphan tail; finalize recovers complete unterminated frames; every finalize write is guarded
...
- The isolated Codex home lives beneath the server startup directory's private `tmp/codex-homes/` tree (root and generated homes `0700`; runtime files `0600`) and copies `auth.json` AND `models_cache.json` from the real `CODEX_HOME`; without the cache every bridge start is a cold Codex install that re-fetches ~280 KB. Homes left by bridges that died without cleanup are swept at startup (`sweepStaleCodexHomes`, 12h), including the legacy OS-temp location — each holds a copy of `auth.json`. Auth write-back is conflict-guarded against the initial isolated snapshot: unchanged or known-invalidated isolated auth is never persisted, and a canonical file changed by a manual login or another bridge is never overwritten
